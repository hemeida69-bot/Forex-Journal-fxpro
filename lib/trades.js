...
function pageToTrade(page) {
  ...
  return {
    ...
    strategy: (p.Strategy && p.Strategy.select && p.Strategy.select.name) || "",
    notes: textOf(p.Notes),
    accountId: (p.Account && p.Account.relation && p.Account.relation[0] && p.Account.relation[0].id) || null,
  };
}
...
function tradeToProperties(t) {
  ...
  if (t.strategy) props.Strategy = { select: { name: t.strategy } };
  if (t.accountId) props.Account = { relation: [{ id: t.accountId }] };
  return props;
}
